...
import { FileText, TrendingUp, Package, Settings } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, providerInfo, isOpen, onClose }) => {
      ...
      <nav className={`no-print fixed inset-0 z-50 md:relative md:flex md:w-64 bg-slate-900 text-white flex-shrink-0 flex-col transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center space-x-3 p-2">
            {providerInfo.logo ? <img src={providerInfo.logo} className="w-8 h-8 rounded-full object-cover" /> : <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center font-bold text-xs">{providerInfo.name.charAt(0)}</div>}
            <div className="text-sm overflow-hidden"><p className="font-medium truncate">{providerInfo.name}</p></div>
          </div>
        </div>
      </nav>
