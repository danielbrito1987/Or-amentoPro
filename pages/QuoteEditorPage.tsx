...
export const QuoteEditorPage: React.FC<QuoteEditorPageProps> = ({ quote, onBack, onSave, onUpdateQuote }) => {
  ...
  const addItem = (item: CatalogItem) => {
    ...
    const newTotal = newItems.reduce((acc, i) => acc + (i.value * i.quantity), 0);
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input type="text" value={quote.customerName} onChange={e => onUpdateQuote({ ...quote, customerName: e.target.value })} className="w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nome do cliente" />
              </div>
              <input type="text" value={quote.customerPhone} onChange={e => onUpdateQuote({ ...quote, customerPhone: maskPhone(e.target.value) })} className="w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="WhatsApp" />
              <input type="email" value={quote.customerEmail} onChange={e => onUpdateQuote({ ...quote, customerEmail: e.target.value })} className="w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="E-mail" />
              <div className="sm:col-span-2 border-t border-gray-100 pt-4 mt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <input type="text" value={quote.customerAddress} onChange={e => onUpdateQuote({ ...quote, customerAddress: e.target.value })} className="sm:col-span-3 w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Rua, número, apto..." />
                  <input type="text" value={quote.customerCity} onChange={e => onUpdateQuote({ ...quote, customerCity: e.target.value })} className="sm:col-span-2 w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Cidade" />
                  <input type="text" value={quote.customerState} onChange={e => onUpdateQuote({ ...quote, customerState: e.target.value })} className="w-full px-4 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="UF" maxLength={2} />
                </div>
              </div>
            </div>
          </div>
                ...
                {quote.items.map((item, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50/50">
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      ...
                      <div className="w-24 text-right font-bold">{formatCurrency(item.value * item.quantity)}</div>
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {catalog.map(item => (
                <div key={item.id} onClick={() => addItem(item)} className="flex items-center justify-between p-3 rounded-xl hover:bg-blue-50 cursor-pointer border border-transparent hover:border-blue-100 transition-all group">
                  <div className="overflow-hidden"><p className="text-sm font-semibold truncate">{item.description}</p><p className="text-xs text-gray-400">{formatCurrency(item.value)}</p></div>
