...
export const LoginPage: React.FC<LoginPageProps> = ({ 
  ...
  // Detecta parâmetro de indicação via URL (?ref=DESIGNER ou ?parceiro=DESIGNER)
  useEffect(() => {
    try {
      ...
      if (ref && ref.trim()) {
        ...
        setMode('register');
        setPartnerCode(cleanRef);
        const res = partnerService.validateCode(cleanRef);
        if (res.valid && res.partner) {
          setPartnerMessage(res.message);
        }
      }
    } catch {}
  }, []);
  ...
  const handlePartnerCodeChange = (val: string) => {
    ...
    if (uppercaseVal.length >= 3) {
      const res = partnerService.validateCode(uppercaseVal, email);
      if (res.valid && res.partner) {
        setPartnerMessage(res.message);
      } else {
              ...
              <div className="bg-slate-950/60 p-3 rounded-2xl border border-slate-800 space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                    ...
                    <span>Código de Indicação de Parceiro (Opcional)</span>
                  </label>
                  <span className="text-[10px] text-blue-400/80 font-medium">Designer / Arquiteto</span>
                </div>
                <div className="relative group">
                ...
                <p className="text-[10px] text-slate-400 leading-normal">
                  Foi indicado por um designer ou parceiro credenciado? Digite o código para vincular sua conta e começar com 7 dias de teste grátis.
                </p>
                {partnerMessage && (
                ...
                {!partnerMessage && partnerCode.length >= 3 && (
                  <p className="text-[11px] text-amber-400 font-semibold flex items-center gap-1 mt-1">
                    ...
                    <span>Código de indicação não encontrado ou inativo.</span>
                  </p>
                )}
              </div>
            )}
